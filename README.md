[代码解读文档.md](https://github.com/user-attachments/files/26433547/default.md)
# 通信网络设计系统 - 完整代码解读
...
**A:** 不同算法有不同需求：
- DFS/BFS 需要快速遍历邻居 → 邻接表 O(degree)
- 查询两点间是否有边 → 邻接矩阵 O(1)
- 空间换时间，提升整体性能

### Q2: 为什么 TSP 不用精确算法？

**A:** TSP 是 NP-hard 问题，精确算法（如动态规划）时间复杂度是 O(n² · 2ⁿ)，当 n > 20 时就无法接受。贪心算法虽然不保证最优，但速度快，适合实时交互。

### Q3: PixiJS 相比 Canvas 2D 有什么优势？

**A:** PixiJS 使用 WebGL 渲染，GPU 加速，性能是 Canvas 2D 的 10-100 倍。对于 200+ 节点的图，Canvas 2D 会卡顿，PixiJS 依然流畅。
...
1. 使用步进模式，逐步观察状态变化
2. 查看日志窗口的文字描述
3. 在代码中添加 `console.log` 输出中间状态
4. 使用浏览器开发者工具的 React DevTools

### Q5: 如何扩展新算法？

**A:**
1. 在 `src/algorithms/` 添加新算法类
2. 在 `ApiServer.cpp` 添加新的 API 路由
3. 在前端 `api.ts` 添加调用函数
4. 在 `AnalysisViews.tsx` 添加新的视图组件
5. 实现算法的逐步执行和状态输出
